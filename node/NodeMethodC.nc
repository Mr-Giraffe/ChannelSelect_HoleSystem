...
module NodeMethodC
{
	uses interface Boot;
	uses interface Leds;
    //uses interface BusyWait<TMicro, uint16_t> as Timer;
    uses interface Alarm<T32khz, uint32_t> as Timer2;
...
/*-------------------task定义及初始化操作 begin----------------------------------*/
	...
	event void Boot.booted()
	{
	    ...
	    if(TOS_NODE_ID < 13) target_id = TOS_NODE_ID + 9;
		else target_id = SINK_ID;
	}
...
/*----------------------------------无线电发送消息的操作-begin----------------------*/
    ...
    event void RadioSend.sendDone(message_t* msg, error_t err)
	{
		...
		ledOp(GREEN, ON);
		ledOp(RED, ON);
		if( mode == DATA_COLLECTION && c_node_id == TOS_NODE_ID){
			call Timer2.start(960);
		}	
	}
	//收到无线电指令，所做的操作
	event message_t* RadioReceive.receive(message_t* msg, void* payload, uint8_t len)
	{
		...
		if(len == sizeof(CommandMsg)){
			...
			if(tempMode != mode || (tempMode == DATA_COLLECTION && c_node_id != tempMessage->nodeId))
			{
				...
				if(mode == START_DETECT)
				{
					...
					if(TOS_NODE_ID%3 == 1){
						curr = 12;
					}else if(TOS_NODE_ID%3 == 2){
						curr = 17;
					}else if(TOS_NODE_ID%3 == 0){
						curr = 22;
					}
					call CC2420Config.setChannel(curr);
				...
				else if(mode == INITIAL)
				{
					...
					c_node_id = tempMessage->nodeId;
					ledOp(RED,TOGGLE);
					ledOp(GREEN,TOGGLE);
					if(c_node_id == TOS_NODE_ID){
						ledOp(RED, OFF);
						ledOp(BLUE, ON);
						ledOp(GREEN, OFF);
						addrOffset = 0;
		...
		}else if(len == sizeof(LocateMsg)){
			call RadioSend.send(target_id, msg, sizeof(LocateMsg));
		}
		return msg;
	}
...
/*-----------------------------------读buffer，传回电脑 begin-----------------------------------*/
	...
	task void dataSendBack()
	{
		...
		if(call RadioSend.send(target_id, &packet, sizeof(LocateMsg)) == SUCCESS)
		{
			radioBusy = TRUE;
